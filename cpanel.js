...
function renderDashMap(rows){const svg=$("#dashChileMap");if(!svg)return;const stats=new Map(dashRegionStats(rows).map(x=>[x.name,x.value])),counts=new Map(dashGroup(rows,o=>o._region).map(x=>[x.name,x.value])),max=Math.max(1,...stats.values());const widths=[62,68,58,67,70,64,74,66,62,70,78,73,82,88,78,92],offsets=[8,0,6,3,10,1,8,0,6,3,7,0,8,4,10,2];let y=12;const shapes=[];DASH_REGIONS.forEach(([region],i)=>{const h=i<5?42:i<10?46:i<13?52:i===13?66:i===14?78:92,w=widths[i],x=88+offsets[i],value=stats.get(region)||0,level=dashRegionLevel(value,max),active=$("#dashRegionFilter")?.value===region;const d=`M${x},${y} q${w*.18},-5 ${w*.36},1 q${w*.22},5 ${w*.64},0 l${-4+((i%3)-1)*3},${h} q-${w*.15},5 -${w*.34},0 q-${w*.24},-5 -${w*.66},1 z`;shapes.push(`<path class="dash-region-shape${active?' active':''}" data-region="${dashSafeText(region)}" d="${d}" fill="${DASH_REGION_COLORS[level]}"><title>${dashSafeText(region)} · ${dashMoney(value)} · ${dashNumber(counts.get(region)||0)} pedidos</title></path><text class="dash-region-label" x="${x+w+12}" y="${y+h*.58}">${dashSafeText(region)}</text>`);y+=h-2});svg.innerHTML=shapes.join("");svg.querySelectorAll('.dash-region-shape').forEach(el=>{el.addEventListener('click',()=>{const r=el.dataset.region||"";if($("#dashRegionFilter")){ $("#dashRegionFilter").value=r; dashboardRegionChanged(); }});el.addEventListener('mousemove',e=>{const tip=$("#dashMapTooltip"),r=el.dataset.region||"",v=stats.get(r)||0,c=counts.get(r)||0;if(!tip)return;tip.innerHTML=`<strong>${dashSafeText(r)}</strong><b>${dashMoney(v)}</b><br>${dashNumber(c)} pedidos`;const rect=el.ownerSVGElement.closest('.dash-map-stage').getBoundingClientRect();tip.style.left=Math.min(rect.width-145,Math.max(8,e.clientX-rect.left+10))+"px";tip.style.top=Math.min(rect.height-70,Math.max(8,e.clientY-rect.top-15))+"px";tip.classList.remove('hidden')});el.addEventListener('mouseleave',()=>$("#dashMapTooltip")?.classList.add('hidden'))})}
