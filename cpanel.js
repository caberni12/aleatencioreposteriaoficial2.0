...
function resetOrderDteImport(){const f=$("#orderDteImportFile");if(f)f.value="";const preview=$("#orderDteImportPreview");if(preview)preview.innerHTML='<i class="bi bi-receipt"></i><span>Selecciona el pedido y el XML. El sistema validará emisor, tipo, folio y receptor antes de guardar.</span>';const label=$("#orderDteImportModal .file-name");if(label)label.textContent="Sin archivo seleccionado"}
...
async function importOrderDte(){const orderId=$("#orderDteImportOrder")?.value||"",file=$("#orderDteImportFile")?.files?.[0];if(!orderId)throw new Error("Selecciona el pedido destino");if(!file)throw new Error("Selecciona el XML del DTE");const xml=await fileAsText(file);const out=await SiiAPI.orderDteImport(xml,String(file.name||"DTE.xml"),orderId,token);const imported=out.documentos||[];const preview=$("#orderDteImportPreview");if(preview)preview.innerHTML=`<i class="bi bi-check-circle-fill"></i><span>${imported.map(x=>`${esc(orderDteName(x.tipo_dte))} · Folio ${Number(x.folio||0).toLocaleString("es-CL")}`).join(" · ")||"DTE importado"}</span>`;siiState.loaded=false;await Promise.all([loadSiiBilling(true).catch(()=>null),loadAdminModules({modules:["orders"],retry:true})]);renderOrders();closeSiiModals();toast(`✓ ${Number(out.importados||imported.length||1)} DTE importado${Number(out.importados||imported.length||1)===1?"":"s"} al pedido`);return out}
